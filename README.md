...
React + MapLibre frontend consumes the REST API. The MBTA publishes its realtime
feeds without authentication, so no API key is required.
...
The second omission matters most. Without it there is no way to express "this
vehicle is between stops 7 and 8, 40% of the way along", which is the basis of
inferring delay from a position. `app.offsets` derives the measure instead:
...
A fraction alone is insufficient. `ST_LineLocatePoint` returns the first nearest
...
figures answer different questions — ours reports how late a vehicle is at this
moment, theirs how late it is expected to be on arrival — which makes the
divergence between them worth measuring.

The figures below predate a correction to the feed join (`with_feed` in
`app/services/delay.py`), which took the newest prediction for a stop rather
than the contemporaneous one. The effects are much larger than the error, so
the findings hold, but treat the exact figures as approximate.

That comparison surfaced a defect in the initial estimator. Broken down by
...
Neither figure was incorrect; they were measuring different things. A bus
stationary at its origin at 04:55 ahead of an 05:00 departure has arrived five
minutes before that stop's scheduled time, so measuring against arrival makes it
appear substantially early. It has not departed, and it will not depart early.
It is on time. Measuring a layover against arrival time is the wrong comparison.
...
Agreement is materially weaker at peak. Service is later on average, and the
...
The residual is systematic rather than noise. The position-derived figure reads
later than the agency's because a prediction absorbs expected recovery —
schedule padding, or time made up on an express segment — while a
position-derived delay reflects only the present moment.
...
moved its prediction to the next stop. Long peak dwells widen the gap. Both
figures are defensible — ours answers "how far behind schedule is this vehicle
now", theirs "when will it next arrive" — but the divergence is a property of
the question, not an error.

`layover` holds at 3s mean absolute divergence under peak load, which is the
clearest evidence that the departure-based rule is the correct one.
...
- The estimator itself is untested. The test suite covers the frontend — the
  delay scale, formatting, chart helpers — but the layover rule, interpolation,
  and the feed join are only verified by the agreement statistics above.
